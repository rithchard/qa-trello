...
test('TC-UI-02 - Sanitización de campos en UI Trello (OWASP A03)', async ({ page }) => {
    ...
    // --- Crear tarjeta con payload XSS ---
    // Payload XSS para prueba
    const xssPayload = '<script>alert("xss")</script>';
    const cardTitle = `Tarjeta XSS ${xssPayload}`;
    ...
    // Comprobar que la última tarjeta contiene el payload XSS como texto
    const lastCard = cardsAfter.nth(countAfter - 1);
    const lastCardText = await lastCard.textContent();
    expect(lastCardText).toContain(xssPayload);

    // Opcional: Validar que no se ejecutó ningún alert (esto depende del entorno de test)
    // No hay forma directa de comprobar ejecución de alert, pero si el texto aparece, no se ejecutó.

    console.log('Última tarjeta creada (sanitización):', lastCardText);
});
